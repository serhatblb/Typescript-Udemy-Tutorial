...
type Employee = Identity & Contact; //Birlestime yapiyoruz

let myPerson:Employee = {
    id:1,
    name: "Serhat",
...
type Customer = BusinessPartner & Contact; //Birlestime yapiyoruz
...
function singContract(partner: BusinessPartner):string {
    let message: string;
...
let isimler = ["Serhat","Ali","Ayşe"]
console.log(getRandomString(isimler));

// ----------------------------- any keyword sayesinde number ve string kullanılabilir  ama any güvenilir değil
// function getRandomElement(items:any[]):any {
//     let randomIndex = Math.floor(Math.random()*items.length) 
//     return items[randomIndex];
// }

function getRandomElement<T>(items:T[]):T { // number,string veya any yerine T verdik
    let randomIndex = Math.floor(Math.random()*items.length) 
    return items[randomIndex];
}

console.log(getRandomElement<number>(numbers)); // istersek < > içinde degeri verebiliriz
console.log(getRandomElement(isimler));

let degiskenlerim = [true,false,false,true]
...
// 3) Interfacelerde Generic Kullanimi
...
// 4) Classlarda Generic Kullanimi
class Stack<T> {
    private elements: T[] = [];
...
let numbersss = new Stack<number>(5);

while(!numbersss.isFull()){
